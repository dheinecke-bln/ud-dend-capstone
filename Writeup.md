...
* some columns need to be converted to the right datatype
...
The ingestion of the data in the first and implemented step is simple. The data was downloaded from the different sources and uploaded to an AWS S3 bucket.
...
This stage represents data that is preprocessed and cleaned. The dataset are not joined yet, but the data types of the selected columns are correct and the data is consistent. Some normalizations have been applied, for example the country codes are all converted to the same ISO format.
...
Finally the analytical components that used the data lake can be build in different ways. In general every application that can handle and query parquet files on S3 is imaginable and it depends on the final needs which solution is the best.
...
* Redshift Spectrum, which allows to query S3 data with SQL using any Redshift client. AWS recommends to store the dimension tables as redshift tables and leave the huge fact tables in S3. Spectrum allows the user to combine this two data formats in on query. [5]
